...
#define rfalCheckEnableObsModeTx()     \
    if (gRFAL.conf.obsvModeTx != 0U) { \
        rfalST25R3911ObsModeTx();      \
    } /*!< Checks if the observation mode is enabled, and applies on ST25R3911 */
#define rfalCheckEnableObsModeRx()     \
    if (gRFAL.conf.obsvModeRx != 0U) { \
        rfalST25R3911ObsModeRx();      \
    } /*!< Checks if the observation mode is enabled, and applies on ST25R3911 */
...
#if RFAL_FEATURE_NFCV
        /*******************************************************************************/
        if ((RFAL_MODE_POLL_NFCV == gRFAL.mode) || (RFAL_MODE_POLL_PICOPASS == gRFAL.mode)) { /* Exchange receive buffer with internal buffer */
...
static void rfalErrorHandling(void)
{
    ...
    if (gRFAL.conf.eHandling == RFAL_ERRORHANDLING_EMVCO) {
        ...
        if (((gRFAL.fifo.bytesTotal + fifoBytesToRead) < RFAL_EMVCO_RX_MAXLEN) && ((gRFAL.TxRx.status == ERR_RF_COLLISION) || (gRFAL.TxRx.status == ERR_FRAMING) || (gRFAL.TxRx.status == ERR_PAR) || (gRFAL.TxRx.status == ERR_CRC) || rxHasIncParError)) {
...
static void rfalPrepareTransceive(void)
{
    ...
    maskInterrupts = (ST25R3911_IRQ_MASK_FWL | ST25R3911_IRQ_MASK_TXE | ST25R3911_IRQ_MASK_RXS | ST25R3911_IRQ_MASK_RXE | ST25R3911_IRQ_MASK_FWL | ST25R3911_IRQ_MASK_NRE | ST25R3911_IRQ_MASK_PAR | ST25R3911_IRQ_MASK_CRC | ST25R3911_IRQ_MASK_ERR1 | ST25R3911_IRQ_MASK_ERR2);
...
ReturnCode rfalListenSetState(rfalLmState newSt)
{
    ...
    do {
        ...
        case RFAL_LM_STATE_POWER_OFF:
            ...
            st25r3911GetInterrupt((ST25R3911_IRQ_MASK_NFCT | ST25R3911_IRQ_MASK_RXS | ST25R3911_IRQ_MASK_CRC | ST25R3911_IRQ_MASK_ERR1 | ST25R3911_IRQ_MASK_ERR2 | ST25R3911_IRQ_MASK_PAR | ST25R3911_IRQ_MASK_EON | ST25R3911_IRQ_MASK_EOF | ST25R3911_IRQ_MASK_RXE));
            ...
            st25r3911EnableInterrupts((ST25R3911_IRQ_MASK_NFCT | ST25R3911_IRQ_MASK_CRC | ST25R3911_IRQ_MASK_ERR1 | ST25R3911_IRQ_MASK_ERR2 | ST25R3911_IRQ_MASK_PAR | ST25R3911_IRQ_MASK_EON | ST25R3911_IRQ_MASK_EOF | ST25R3911_IRQ_MASK_RXE));
...
ReturnCode rfalWakeUpModeStart(const rfalWakeUpConfig *config)
{
    ...
    if ((gRFAL.wum.cfg.cap.enabled && (gRFAL.wum.cfg.indAmp.enabled || gRFAL.wum.cfg.indPha.enabled)) || (!gRFAL.wum.cfg.cap.enabled && !gRFAL.wum.cfg.indAmp.enabled && !gRFAL.wum.cfg.indPha.enabled)) {
        return ERR_PARAM;
    }
...
static void rfalRunWakeUpModeWorker(void)
{
    ...
    case RFAL_WUM_STATE_ENABLED_WOKE:
        // irqs = st25r3911GetInterrupt(( ST25R3911_IRQ_MASK_WT | ST25R3911_IRQ_MASK_WAM | ST25R3911_IRQ_MASK_WCAP ));
